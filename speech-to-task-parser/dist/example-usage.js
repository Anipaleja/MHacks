...
const speech_to_task_parser_1 = __importDefault(require("./speech-to-task-parser"));
async function main() {
    // Initialize the parser with your Gemini API key
    const apiKey = process.env.GEMINI_API_KEY || 'your-gemini-api-key-here';
    const parser = new speech_to_task_parser_1.default(apiKey);
    // Example scenarios
    const testCases = [
    ...
    try {
        await parser.startContinuousListening((result) => {
            ...
            if (result.type === 'direct_command') {
                ...
                console.log(`Suggestion: ${result.suggestion ?? 'No suggestion available'}`);
